...
int s2o_sha256_encrypt2(char* src, char* dest)
{   
    CSha256 c = {0,};   
    ...
    Sha256_Init(&c);   
    Sha256_Update(&c, (Byte*)src, strlen(src));   
    /*Sha256_Update(&c, (Byte*)"itops01s2orhkswp!", strlen("itops01s2orhkswp!"));   */
    /*Sha256_Update(&c, (Byte*)"itops01itops000", strlen("itops01itops000")); */
    Sha256_Final(&c, digest); 

	for(i=0;i<32;i++) {
		memset(buf, 0, sizeof buf);
		Char2Hex(digest[i], buf);
		strcat(dest, buf);
	}
    return 0;   
}  
