...
#elif defined(KISA_LITTLE_ENDIAN)
...
void SHA256_Transform(ULONG_PTR Message, ULONG_PTR ChainVar, IN BYTE *pszDigest, int type)
{
	ULONG a, b, c, d, e, f, g, h, T1, X[64];
	ULONG j;
	/*int a, b, c, d, e, f, g, h, T1, X[64];*/
	/*int j;*/
...
#define FF(a, b, c, d, e, f, g, h, j) {							\
	...
	if (type == 1) {
		sprintf(pszDigest, "%x%x%x%x%x%x%x%x", (unsigned int)ChainVar[0], (unsigned int)ChainVar[1], (unsigned int)ChainVar[2]
							, (unsigned int)ChainVar[3], (unsigned int)ChainVar[4], (unsigned int)ChainVar[5]
							, (unsigned int)ChainVar[6], (unsigned int)ChainVar[7]);
		/*sprintf(pszDigest, "%x%x%x%x%x%x%x%x",(unsigned int)ChainVar[0],ChainVar[1],ChainVar[2],ChainVar[3],ChainVar[4],ChainVar[5],ChainVar[6],ChainVar[7]);*/
	}
}
/*
*********************************************************************************************************************************
...
*********************************************************************************************************************************
*/
void SHA256_Process( OUT SHA256_INFO *Info, IN const BYTE *pszMessage, IN UINT uDataLen, OUT BYTE *pszDigest )
{
	...
	while (uDataLen >= SHA256_DIGEST_BLOCKLEN)
	{
		...
		SHA256_Transform((ULONG_PTR)Info->szBuffer, (ULONG_PTR)Info->uChainVar, pszDigest, 0);
...
void SHA256_Close( OUT SHA256_INFO *Info, IN BYTE *pszDigest )
{
	...
	SHA256_Transform((ULONG_PTR)Info->szBuffer, (ULONG_PTR)Info->uChainVar, pszDigest, 1);
...
void SHA256_Encrpyt( IN const BYTE *pszMessage, IN UINT uPlainTextLen, OUT BYTE *pszDigest )
{
	...
	SHA256_Process( &info, pszMessage, uPlainTextLen , pszDigest);
